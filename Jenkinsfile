    ...
    stage('Scan Docker Image for Vulnerabilities') {
      steps {
        script {
          ...
          def vulnerabilities = sh(script: "trivy image  --exit-code 0 --ignore-unfixed --severity HIGH,MEDIUM,LOW --format template --template '@html.tpl' -o reports/image-scan.html --no-progress ${registry}:${env.BUILD_ID}", returnStdout: true).trim()
